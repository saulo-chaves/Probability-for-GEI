...
setwd("/media/kaio/4063cb1f-cab9-4047-a9f0-29064b342919/kaio/DRIVE_UFV/JOB/Papers/AA_Publicados/Paper_Bayes_Model_GxE/Figures_DONE/Bayes_All_plots")
...
out_M4 <- rstan::extract(Model4, permuted = TRUE)
print(names(out_M4))
...
# Yi.
g_i = get_map(out_M4$g) #+ mean(df$GY)

# Vi
n_g = nlevels(df$H)  # Genotype
n_l = nlevels(df$L)  # Environment
g_l = get_map(out_M4$gl)
...
Risk = data.frame(Risk, ID = paste("G", 1:36, sep = ""))
